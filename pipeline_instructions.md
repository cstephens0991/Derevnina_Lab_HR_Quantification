...
Use the ImageQuant 800 to collect fluorescence images of agroinfiltrated leaves. The ImageQuant imager can be found on the far right-hand corner of the main lab (CSF-023). 

![ImageQuant 800 imager, terminal and monitor. Turn on both the imager and the computer terminal (red arrows).](./md_files/0_ImageQuant-imager.jpg)
...
![ImageQuant software homescreen. Select "Fuorescence".](./md_files/1_home-screen.jpg)
...
The "benthi_leaves" or "LD LEAVES" settings are normally used for _N. benthamiana_ leaves, whilst "Lettuce_leaves" is available for imaging lettuce. A setting including the Cy3 and/or IRlong channels is recommended, as these provide the best contrast of cell death to background leaf. Please note that, if using the “Lettuce_leaves” setting, image collection time is ~3 minutes. 

![Fluorescence imaging homescreen. Select "Open method".](./md_files/2_fluorescence-screen.jpg)

![Recommended methods include "benthi_leaves", "LD LEAVES" or "Lettuce_leaves". The "Fluor.multi / Dyes" column provides information on the channels used for each method.](./md_files/3_channel-options.jpg)
...
![Open the front of the ImageQuant and place a leaf on the imaging tray. This should be in the lower position of the imager.](./md_files/5_ImageQuant-tray.jpg)

- _Note: for some plant species, it may be prefereable to image the underside ("abaxial" side) of the leaf. If this is the case, the leaf images may be later horizontally inverted, to standardise the positioning of the leaf patches. See "Flip images of leaf abaxial sides" for further details._
...
![Infiltrated leaf example](./md_files/7_example-leaf-layout.jpg)
...
On your own PC, open ImageJ / Fiji. ImageJ is available to download at `https://imagej.net/ij/download.html`. Use “Plugins > Macros > Run…” to run the macro script “1_Move_image_files.ijm” to move greyscale channel images to a new folder for image processing.
-	The script will first prompt you to navigate to and select the parent folder containing images.

![Navigate to the parent folder containing your image folders.](./md_files/8_move-image-files.jpg)
...
<video src="./HR_scoring.mp4" width="320" height="240" controls></video>
